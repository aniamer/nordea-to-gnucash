 ...
 * @see priv.nordea.db.hib.Jobs
 * @author Hibernate Tools
 */
public class JobsHome {

	private static final Logger log = Logger.getLogger(JobsHome.class);
	...
	public Jobs merge(Jobs detachedInstance) {
		...
		try {
			Jobs result = (Jobs) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Jobs findById(java.lang.String id) {
		...
		try {
			Jobs instance = (Jobs) sessionFactory.getCurrentSession().get(
					"priv.nordea.db.hib.Jobs", id);
			if (instance == null) {
	...
	public List<Jobs> findByExample(Jobs instance) {
		log.debug("finding Jobs instance by example");
		try {
			List<Jobs> results = (List<Jobs>) sessionFactory
					.getCurrentSession()
					.createCriteria("priv.nordea.db.hib.Jobs")
