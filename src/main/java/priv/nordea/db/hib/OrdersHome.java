 ...
 * @see priv.nordea.db.hib.Orders
 * @author Hibernate Tools
 */
public class OrdersHome {

	private static final Logger log = Logger.getLogger(OrdersHome.class);
	...
	public Orders merge(Orders detachedInstance) {
		...
		try {
			Orders result = (Orders) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public List<Orders> findByExample(Orders instance) {
		...
		try {
			List<Orders> results = (List<Orders>) sessionFactory
					.getCurrentSession()
					.createCriteria("priv.nordea.db.hib.Orders")
